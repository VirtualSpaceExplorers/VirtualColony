Shader "Custom/Mars Skybox Shader"
{
...
Properties {
    ...
    _DustThickness ("Dust Thickness", Range(0,5)) = 1.0
    _DustTint ("Dust Tint", Color) = (.5, .5, .5, 1)
...
SubShader {
    ...
    Pass {
        ...
        uniform half3 _DustTint;
        uniform half _DustThickness;
...
//              for(int i=0; i<int(kSamples); i++) // Loop removed because we kept hitting SM2.0 temp variable limits. Doesn't affect the image too much.
        ...
        // Mars: fade in dust tint around horizon
            OUT.skyColor = lerp(OUT.skyColor, _DustTint, 
                clamp(_DustThickness*pow(1.05-eyeRay.y,1),0.0,1.0)); 
            if (eyeRay.y<0.0) OUT.skyColor = _DustTint;
        ...
        // if y < 0 [eyeRay.y > 0] - sky
            ...
            // we did precalculate color in vprog
            col = IN.skyColor; 
